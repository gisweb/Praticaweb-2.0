	...
	foreach ($_POST as $key=>$value){
		...
		elseif ($tab=="doc"){			
			...
			if (!$_SESSION["ADD_NEW"]){//inserisco solo se non ho già inserito il dato
				$sql="insert into pe.allegati (pratica,documento,protocollo,data_protocollo,$value,chk,uidins,tmsins) values ($idpratica,$id,'$prot','$data_prot'::date,1,1,".$_SESSION["USER_ID"].",".time().");";
				//print_debug($sql);
				$db->sql_query ($sql);
				if ($value=="mancante")
					$flag_mancante=1;
