...
//la variabile ruolo contiene il ruolo corrente per il nominativo
	...
	elseif ($_POST["azione"]=="Annulla"){
		...
		if ($modo=="edit")
			...
			$active_form="vigi.elenco_soggetti.php?pratica=$idpratica&id=$id";
	}
		
	elseif (ereg("Sposta",$_POST["azione"])){
		$db = new sql_db(DB_HOST.":".DB_PORT,DB_USER,DB_PWD,DB_NAME, false);
		if(!$db->db_connect_id)  die( "Impossibile connettersi al database");
		$sql="update vigi.soggetti set $ruolo=-1 where id=$id;";		
		if (DEBUG) echo $sql;
		$db->sql_query ($sql);
		$active_form="vigi.elenco_soggetti.php?pratica=$idpratica";
	}

	elseif ($_POST["azione"]=="Elimina"){
		$db = new sql_db(DB_HOST.":".DB_PORT,DB_USER,DB_PWD,DB_NAME, false);
		if(!$db->db_connect_id)  die( "Impossibile connettersi al database");
		$sql="update vigi.soggetti set $ruolo=0 where id=$id;
				  delete from vigi.soggetti where proprietario=0 and richiedente=0 and concessionario=0 
