...
//restituisce il controllo in funzione di tipo letto dal configfile e lo riempie con i dati il valore w pu� contenere pi� informazioni
	$retval=null; 
	$class=null;
	$help=null;
	$onChange=null;
...
function get_riga_edit($nriga,$frozen_cols=Array()){
	$ctr='';
//prendo una riga che pu� essere fatta da uno,  due o pi� colonne
// restituisce la riga in modalit� edit con label controllo associato
...
// restituisce la riga in modalit� view
	$testo_riga='';
	$riga=$this->tab_config[$nriga];
	for ($i=0;$i<count($riga);$i++){
		list($label,$campo,$w,$tipo,$html5Data)=explode(';',$riga[$i]);
