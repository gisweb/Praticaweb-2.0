...
/*---------------------------------------------------------------------------------------------*/    
	...
	static function getComunicazione($id=0){
		...
		if($stmt->execute(Array($id))){
			...
			$sql = "SELECT B.id,B.pec FROM pe.comunicazioni A LEFT JOIN pe.soggetti B USING(pratica) WHERE B.id::varchar = ANY(destinatari) AND A.id=? ";
			$stmt = $dbh->prepare($sql);
			if ($stmt->execute(Array($id))){
				$tmp = $stmt->fetchAll(PDO::FETCH_ASSOC);
				for($i=0;$i<count($res);$i++) $destinatari[]=$tmp[$i]["pec"];
			}
			else{
				$destinatari = Array();
			}
			//RECUPERO Allegati da Inviare
			$idAllegati = explode(',',str_replace('{','',str_replace('}','',$comunicazione['allegat1_1'])));
			$allegati = Array();
			for($i=0;$i<count($idAllegati);$i++){
				$a = self::getDocumento($idAllegati[$i],$pratica,0);
				if ($a["success"]==1) $allegati[]=Array("file"=>$a["file"],"name"=>$a["name"]);
			}
			//RECUPERO Documenti da Inviare
			$idStampe = explode(',',str_replace('{','',str_replace('}','',$comunicazione['allegat1_1'])));
			for($i=0;$i<count($idStampe);$i++){
				$a = self::getDocumento($idStampe[$i],$pratica,1);
				if ($a["success"]==1) $allegati[]=Array("file"=>$a["file"],"name"=>$a["name"]);
			}
