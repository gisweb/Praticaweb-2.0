...
if (isset($_POST["azione"]) && $_POST["azione"]){
	...
	if (isset($array_dati["errors"])) //sono al ritorno errore
		...
		include "db/db.storage.iter.php";
	}
...
if (($modo=="edit") or ($modo=="new") ){
	//---<<<<<<<<<<<<<<<<<<<<<<<<<<<<<  EDITA ELENCO ITER >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>------------------------------>
		...
		<form method="post" name="iter" action="vigi.iter.php">		
		...
		<tr> 
				  ...
				  <table cellPadding="2" border="0" class="stiletabella">
					...
					<tr>
						...
						<td  valign="top">
							...
							<input  class="hexfield1" style="width:130px" type="submit" value="Carica Documento" onclick="NewWindow('stp.carica_documento.php?schema=vigi&pratica=<?=$idpratica?>','documento',500,200);" >
						</td>
					</tr>
				</table>
		  ...
		  <tr> 
			<td> 
				...
				<INPUT type="hidden" name="config_file" value="vigi/iter.tab">
				<input name="active_form" type="hidden" value="vigi.iter.php">
		...
		<FORM method="post" action="/praticaweb.php">	
			<tr>
				<td><input name="active_form" type="hidden" value="storage.iter.php">
